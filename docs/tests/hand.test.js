...
import { pinchRatio, isPinching, penPoint, Debouncer, LM } from "../hand.js";
...
test("spread-apart fingers are not a pinch", () => {
  const lm = baseLandmarks();
  lm[LM.INDEX_MCP] = { x: 0.4, y: 0.5, z: 0 };
  lm[LM.PINKY_MCP] = { x: 0.6, y: 0.5, z: 0 }; // palm width 0.2
  lm[LM.THUMB_TIP] = { x: 0.0, y: 0.5, z: 0 };
  lm[LM.INDEX_TIP] = { x: 1.0, y: 0.5, z: 0 }; // far apart, ratio = 1/0.2 = 5
  assert.equal(isPinching(lm), false);
});

test("penPoint is the midpoint of thumb and index tips", () => {
  const lm = baseLandmarks();
  lm[LM.THUMB_TIP] = { x: 0.2, y: 0.4, z: 0 };
  lm[LM.INDEX_TIP] = { x: 0.6, y: 0.8, z: 0 };
  const p = penPoint(lm);
